...
router.get("/superhero", (req,res) => {
    Adventure.find({genre: "superhero"}).sort({date: -1}).then(results => res.json(results))
})

router.get("/villain", (req,res) => {
    Adventure.find({genre: "villain"}).sort({date: -1}).then(results => res.json(results))
})

router.get("/random", (req,res) => {
    Adventure.find({genre: "random"}).sort({date: -1}).then(results => res.json(results))
})
